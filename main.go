...
func (m *monitor) updateTarget(ip string, up bool, latency int64) {
	...
	if up {
		if !t.IsUp {
			t.StatusChangedAt = time.Now().Format("2006-01-02 15:04:05")
		}
		t.IsUp = true
		t.Received++
		...
		t.LastOnlineAt = time.Now().Format("2006-01-02 15:04:05")
	} else {
		if t.IsUp {
			t.StatusChangedAt = time.Now().Format("2006-01-02 15:04:05")
		}
		t.IsUp = false
		t.LastOfflineAt = time.Now().Format("2006-01-02 15:04:05")
	}
